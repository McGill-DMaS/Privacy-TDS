    ...
    for (int v = 0; v < nVIDs; ++v) {
        ...
        if (!pSplitAttrib || pSplitAttrib->isRelated(pVID)) {
            if (pParentPartition) {
                ...
                if (!pVID->updateVPathCount(vidPath, pParentPartition->getNumRecords() * -1, false))
                    return false;
            ...
            if (pChildPartitions) {
                ...
                for (POSITION pos = pChildPartitions->GetHeadPosition(); pos != NULL;) {
                    ...
                    if (!pVID->updateVPathCount(vidPath, pChildPartition->getNumRecords(), false))
                        return false;
                }
            }
        }
...
bool CTDPartitioner::updateVIDTreesMinAC()
{
    ...
    int nVIDs = pVIDs->GetSize();
    for (int v = 0; v < nVIDs; ++v) {
        if (!pVIDs->GetAt(v)->updateMinACNode())
            return false;
    }
    return true;
}
...
bool CTDPartitioner::computeGR_ALratios()
{
...
#ifdef _DEBUG_PRT_INFO
        cout << _T("VID #") << v << _T(" AC_Before_Split=") << pVID->m_pMinACNode->getCount() << _T(" k=") << pVID->m_kAnonymity << endl;
#endif
    }    
    ...
    for (int a = 0; a < nAttribs; ++a) {
        ...
        for (POSITION posCut = pAttrib->m_cut.GetHeadPosition(); posCut != NULL;) {
            ...
            // 2. Compute A(VIDv, S) and A(VIDv) - A(VIDv, S).
            // 3. Rollback VID trees.
            sumACsDiffs = 0;
            for (v = 0; v < nVIDs; ++v) {
                if (!relatedToVID[v]) {
                    // A(VIDv) - A(VIDv, S) must be 0.
                ...
                // 1. Check whether k is violated in each VID.
                pVID = pVIDs->GetAt(v);
                if (!pVID->searchMinAC(minACafterS))
                    return false;
                if (minACafterS < pVID->m_kAnonymity) {
                                          ...
                                          CTDVirtualID*  pVID)
{
    ...
    if (!pVID->updateVPathCount(vidParentPath, pParentPartition->getNumRecords() * -1, true)) {
        ASSERT(false);
        return false;
    }
    ...
    for (int c = 0; c < nChildConcepts; ++c) {
        ...
        // Update the tree.
        if (!pVID->updateVPathCount(vidPathNextLevel, rc, true)) {
            ASSERT(false);
            return false;
        }        
    }
