...
{
    if (nArgs != 7 || !argv) {
        cout << _T("Usage: PrivacyTD <dataSetName> <bRemoveUnknownOnly> <bIncludeNonVA> <defaultK> <nInputRecs> <nTraining>") << endl;
        return false;
    }
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	else
	{
        ...
        CString dataSetName;
        int defaultK = 0, nTraining = 0, nInputRecs = 0;
        bool bRemoveUnknownOnly = false, bIncludeNonVA = false;

        if (!parseArgs(argc, argv, dataSetName, bRemoveUnknownOnly, bIncludeNonVA, defaultK, nInputRecs, nTraining)) {
